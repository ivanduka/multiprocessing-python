...
#######################FUNCTIONS##################################################
# pd.DataFrame:
def get_table_titles(page: int, file, df: pd.DataFrame) -> list():
    tbl_names_trunc = list()
    tbl_names = list(df[(df['page_number'] == page) & (
        df['DataID_pdf'] == file)]['final_table_title'])
...
def extract_table(argument_list):
    ...
    file_path = 'F:/Environmental Baseline Data/Version 4 - Final/PDF/{}'.format(
        file)
    ...
    try:
        ...
        for ind, page in enumerate(pages):
            ...
            try:
                tables = camelot.read_pdf(file_path, pages=str(pg_num+1), flag_size=True, copy_text=[
            ...
            # one table on that page, identify this as figure and continute the loop and go to the next page
                ...
                # add pdf ID!!!!
                print(
                    f"Page {(tables[0].parsing_report)['page']} contains an image")
                continue
            # in case only one table is present on the page,
                ...
                if len(title_lst) == 0:
                    ...
                    if (pg_num-1 in title_dict):
                        ...
                        col_join_curr = (ccc.replace(' ', '')
                                         ).replace('\n', '')
                        col_concat_prev = list(lst_tbl_df.columns.values)
                        ccp = ''.join(col_concat_prev)
                        col_join_prev = (ccp.replace(' ', '')
                                         ).replace('\n', '')
                        ratio_similarity = fuzz.token_sort_ratio(
                            col_join_curr, col_join_prev)
                        ...
                        if (len((set(lst_tbl_df.columns)).difference(set(df_tb.columns))) == 0) or (len(set(lst_tbl_df.columns)) == len(set(df_tb.columns))) or (ratio_similarity > 89):
                            ...
                            xlsx_name = file_name + '_' + xl_name + \
                                '_'+str(pg_num+1)+'_'+str(1) + '.csv'
                            df_tb.to_csv(xlsx_name, index=False,
                                         encoding='utf-8-sig')
                        else:
                            xl_name = file_name
                            chars.append([0, df_tb, xl_name])
                            xlsx_name = file_name + '_' + \
                                str(pg_num+1)+'_'+str(1) + '.csv'
                            df_tb.to_csv(xlsx_name, index=False,
                                         encoding='utf-8-sig')
...
df = pd.read_csv(path, usecols=['page_number', 'final_table_title', 'Application title short',
                                'DataID_pdf',                                   'categories', 'Category'])
...
if __name__ == "__main__":
    ...
    ngtl2021 = df[df['Application title short']
                  == hearing].reset_index(drop=True)
    ngtl2021.head()
...
komie_north = df[df['Application title short']
                 == hearing].reset_index(drop=True)
komie_north.head()
...
edmonton_hardisty = df[df['Application title short']
                       == hearing].reset_index(drop=True)
...
leismer2kettle = df[df['Application title short']
                    == hearing].reset_index(drop=True)
leismer2kettle.head()
# Change this folder to the path were you want the tables saved
...
groundbirch = df[df['Application title short']
                 == hearing].reset_index(drop=True)
groundbirch.head()
# Change this folder to the path were you want the tables saved
...
horn_river = df[df['Application title short']
                == hearing].reset_index(drop=True)
horn_river.head()
# Change this folder to the path were you want the tables saved
...
northwest_mainline = df[df['Application title short']
                        == hearing].reset_index(drop=True)
...
cushing_expansion = df[df['Application title short']
                       == hearing].reset_index(drop=True)
...
red_willow = df[df['Application title short']
                == hearing].reset_index(drop=True)
red_willow.head()
# Change this folder to the path were you want the tables saved
...
south_peace = df[df['Application title short']
                 == hearing].reset_index(drop=True)
south_peace.head()
# Change this folder to the path were you want the tables saved
...
keystoneXL = df[df['Application title short']
                == hearing].reset_index(drop=True)
keystoneXL.head()
# Change this folder to the path were you want the tables saved
...
alberta_clipper = df[df['Application title short']
                     == hearing].reset_index(drop=True)
...
southern_lights = df[df['Application title short']
                     == hearing].reset_index(drop=True)
...
north_corridor = df[df['Application title short']
                    == hearing].reset_index(drop=True)
north_corridor.head()
# Change this folder to the path were you want the tables saved
os.chdir(r'F:\Environmental Baseline Data\Version 4 - Final\CSV\north_corridor')
...
edson_mainline = df[df['Application title short']
                    == hearing].reset_index(drop=True)
edson_mainline.head()
# Change this folder to the path were you want the tables saved
...
deep_panuke = df[df['Application title short']
                 == hearing].reset_index(drop=True)
deep_panuke.head()
# Change this folder to the path were you want the tables saved
...
goldboro_abandonment = df[df['Application title short']
                          == hearing].reset_index(drop=True)
...
spruce_ridge = df[df['Application title short']
                  == hearing].reset_index(drop=True)
spruce_ridge.head()
# Change this folder to the path were you want the tables saved
...
energy_east = df[df['Application title short']
                 == hearing].reset_index(drop=True)
energy_east.head()
# Change this folder to the path were you want the tables saved
...
files = ['2968357.pdf', '2968358.pdf', '2968365.pdf', '2968366.pdf', '2968367.pdf', '2968368.pdf', '2968370.pdf', '2968373.pdf', '2968377.pdf', '2968378.pdf', '2968379.pdf', '2968380.pdf', '2968417.pdf', '2968421.pdf', '2968422.pdf', '2968425.pdf', '2968426.pdf', '2968427.pdf', '2968430.pdf', '2968431.pdf', '2968432.pdf', '2968434.pdf', '2968436.pdf', '2968440.pdf', '2968448.pdf', '2968450.pdf', '2968452.pdf', '2968457.pdf', '2968458.pdf', '2968462.pdf', '2968464.pdf', '2968467.pdf', '2968468.pdf', '2968470.pdf', '2968475.pdf', '2968476.pdf', '2968477.pdf', '2968478.pdf', '2968479.pdf', '2968480.pdf', '2969255.pdf', '2969256.pdf', '2969257.pdf', '2969269.pdf', '2969272.pdf', '2969283.pdf', '2969358.pdf', '2969359.pdf', '2969375.pdf', '2969376.pdf', '2969377.pdf', '2969378.pdf', '2969379.pdf',
         '2969456.pdf', '2969457.pdf', '2969460.pdf', '2969462.pdf', '2969464.pdf', '2969466.pdf', '2969469.pdf', '2969470.pdf', '2969473.pdf', '2969475.pdf', '2969476.pdf', '2969477.pdf', '2969479.pdf', '2969480.pdf', '2969481.pdf', '2969496.pdf', '2969497.pdf', '2969557.pdf', '2969564.pdf', '2969567.pdf', '2969569.pdf', '2969570.pdf', '2969571.pdf', '2969586.pdf', '2969587.pdf', '2969659.pdf', '2969663.pdf', '2969665.pdf', '2969666.pdf', '2969674.pdf', '2969675.pdf', '2969678.pdf', '2969679.pdf', '2969680.pdf', '2969688.pdf', '2969689.pdf', '2969759.pdf', '2969763.pdf', '2969765.pdf', '2969766.pdf', '2969767.pdf', '2969780.pdf', '2969781.pdf', '2969782.pdf', '2969783.pdf', '2969865.pdf', '2969866.pdf', '2969880.pdf', '2969881.pdf', '2969882.pdf', '2969968.pdf', '2969969.pdf', '2969970.pdf']
...
north_montney = df[df['Application title short']
                   == hearing].reset_index(drop=True)
north_montney.head()
# Change this folder to the path were you want the tables saved
os.chdir(r'F:\Environmental Baseline Data\Version 4 - Final\CSV\north_montney')
...
trans_mountain = df[df['Application title short']
                    == hearing].reset_index(drop=True)
trans_mountain.head()
# Change this folder to the path were you want the tables saved
...
carmon_creek = df[df['Application title short']
                  == hearing].reset_index(drop=True)
carmon_creek.head()
# Change this folder to the path were you want the tables saved
...
eastern_mainline = df[df['Application title short']
                      == hearing].reset_index(drop=True)
...
towerbirch = df[df['Application title short']
                == hearing].reset_index(drop=True)
towerbirch.head()
# Change this folder to the path were you want the tables saved
